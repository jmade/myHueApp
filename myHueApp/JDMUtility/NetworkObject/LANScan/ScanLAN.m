...
//    NSLog(@"self.localAddress:%@",self.localAddress);
//    NSLog(@"self.netMask:%@",self.netMask);
    ...
    if ([self isIpAddressValid:self.localAddress] && (a.count == 4) && (b.count == 4)) {
        for (int i = 0; i<4; i++) {
            ...
            if (!self.baseAddress.length) {
                self.baseAddress = [NSString stringWithFormat:@"%ld", (long)and];
            }
            else {
                self.baseAddress = [NSString stringWithFormat:@"%@.%ld", self.baseAddress, (long)and];
        ...
        NSLog(@"Starting Scan At Interval:%f",self.scanTime);
        
        dispatch_async(dispatch_get_main_queue(), ^
                       {
                           self.timer = [NSTimer scheduledTimerWithTimeInterval:self.scanTime target:self selector:@selector(pingAddress) userInfo:nil repeats:YES];
                       });
